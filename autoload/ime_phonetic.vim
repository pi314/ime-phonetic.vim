...
function! ime_phonetic#_SearchWord (code_list) " {{{
...
function! ime_phonetic#_GetBestWord (code_list) " {{{
...
function! ime_phonetic#_FindBestSentence (code_list) " {{{
...
function! ime_phonetic#_QueryOneChar (code_list) " {{{
    ...
    let l:result = []
    for l:key in ime_phonetic#_GetAllKeyStartsWith(s:table, a:code_list[0])
